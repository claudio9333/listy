  ...
  void _autoSave() async {
    ...
    if (widget.fileIndex < allFiles.length) {
      allFiles[widget.fileIndex] = {
        ...
        'data': _currentData, // Salva i dati correnti, inclusi quelli nei textbox [cite: 2026-02-01]
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: _isSelectionMode
            ? IconButton(icon: const Icon(Icons.close), onPressed: () => setState(() => _selectedRows.clear()))
            : null,
        title: _isSelectionMode
            ? Text("${_selectedRows.length} selezionati")
            : (_isSearching ? _buildModernSearchField(colorScheme, displayColumns) : Text(_currentFileName)),
        actions: [
          if (_isSelectionMode)
            IconButton(icon: const Icon(Icons.delete_sweep_rounded), onPressed: _deleteSelectedRows)
          else ...[
            IconButton(
          ...
          Expanded(
            child:  _currentData.isEmpty
              ...
              itemBuilder: (context, index) {
                ...
                return InkWell(
                  // Attiva la modalità con la pressione prolungata
                  onLongPress: () => _toggleSelection(index),

                  // Se siamo in modalità selezione, basta un click. Altrimenti apre il dettaglio.
                  onTap: _isSelectionMode
                      ? () => _toggleSelection(index)
                      : () => Navigator.push(context, MaterialPageRoute(
                      builder: (_) => CsvDetailPage(record: _currentData[index], allColumns: _allColumns))),
      ...
      child: Row(
        children: [
          DropdownButton<String>(
            ...
            items: displayColumns.map((c) => DropdownMenuItem(value: c, child: Text(c.toUpperCase()))).toList(),
            onChanged: (v) => setState(() { _searchColumn = v!; _performSearch(_searchController.text); }),
          ),
      ...
      child: Row(
        children: [
          ...columns.map((col) {
            ...
            return Expanded(
              child: Text(value, style: theme.textTheme.bodyMedium?.copyWith(color: colorScheme.onSurface.withOpacity(0.8))),
            );
          }).toList(),
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            const Text("Tipo di input da tastiera", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 12),
                            SegmentedButton<String>(
                              segments: const [
                                ButtonSegment(value: 'off', label: Text("Disabilitato"), icon: Icon(Icons.edit_off_sharp)),
